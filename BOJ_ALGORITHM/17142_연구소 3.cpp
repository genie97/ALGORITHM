...
int bfs() {
	...
	while (!q.empty()) {
		if (space == clean_space) //퍼진 공간 개수랑 깨끗했던 공간 개수 같으면 다 퍼진 것!
			return time;
		size = q.size();
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < 4; j ++) {
				...
				if (temp[nx][ny] == 0 || temp[nx][ny] == 2) { //2인 경우는 활성화 시켜주기
					if (temp[nx][ny] == 0) space++; //0일때는 공간 늘리기
					temp[nx][ny] = 3; //퍼진곳 
					q.push(Pos{ nx,ny });
				}
			}
...
void make_comb(int idx, int cnt) {
	if (cnt == M) {
		...
		for (int i = 0; i < vt.size(); i++) {
			int x = vt[i].x;
			int y = vt[i].y;
			temp[x][y] = 3; //활성시킨 바이러스는 3으로 바꿔줄 것
		}
		int spread_time = 0;
		spread_time = bfs();
...
int main() {
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			...
			if (map[i][j] == 0) //다 퍼진 거 체크하기 위해 바이러스 없는 
				clean_space++;
		}
	}
	make_comb(0, 0);
