...
int space = 0; // 0인공간 개수(0인 공간이 다 차면 퍼진 시간 리턴)
...
int bfs() {
	queue<Pos> q;
	int virus_num = M;
	int spread_num = 0; //바이러스 생긴 공간 개수
	...
	while (!q.empty()) {
		int size = q.size();
		if (spread_num == (space + virus.size()-M)) // 0이있는 공간 + (바이러스가능 공간-바이러스 개수) => 다퍼졌다
			return spread_time;
...
void make_comb(int idx, int cnt) {
	if (cnt == M) {
		...
		//최소값 구하기
		spread_ans = bfs();
		res = min(res, spread_ans); // 여기서 min값 갖기
